...
{
    draw_text(__view_get( e__VW.XView, 0 )+9, __view_get( e__VW.YView, 0 )+100, string_hash_to_newline(global.Label));
    draw_text(__view_get( e__VW.XView, 0 )+9, __view_get( e__VW.YView, 0 )+116, string_hash_to_newline(textEdit));
}
else if (status == EDIT_MSG)
{
    draw_text(__view_get( e__VW.XView, 0 )+9, __view_get( e__VW.YView, 0 )+100, string_hash_to_newline(global.Message));
    draw_text(__view_get( e__VW.XView, 0 )+9, __view_get( e__VW.YView, 0 )+116, string_hash_to_newline(textEdit));
}
...
{
    draw_text(__view_get( e__VW.XView, 0 )+9, __view_get( e__VW.YView, 0 )+116, string_hash_to_newline(global.Music + music));
}
...
{
    strLen = string_length(global.SaveLevelYN)*8;
    n = 320 - strLen;
    n = ceil(n / 2);
    draw_text(__view_get( e__VW.XView, 0 )+n, __view_get( e__VW.YView, 0 )+116, string_hash_to_newline(global.SaveLevelYN));
    
    m = 16;
    draw_set_color(c_yellow);
    if not (instance_exists(oEntrance))
    {
        strLen = string_length(global.WarningNoEntrance)*8;
        n = 320 - strLen;
        n = ceil(n / 2);
        draw_text(__view_get( e__VW.XView, 0 )+n, __view_get( e__VW.YView, 0 )+116+m, string_hash_to_newline(global.WarningNoEntrance));
        m += 8;
    }
    
    if not (instance_exists(oExit))
    {
        strLen = string_length(global.WarningNoExit)*8;
        n = 320 - strLen;
        n = ceil(n / 2);
        draw_text(__view_get( e__VW.XView, 0 )+n, __view_get( e__VW.YView, 0 )+116+m, string_hash_to_newline(global.WarningNoExit));
    }
}
else
{
