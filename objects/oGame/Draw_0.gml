    ...
    if (oPlayer1.dead or isRoom("rMoon"))
    {
        if (isLevel())
        {
            if (drawStatus > 0)
            {
                ...
                draw_text(__view_get( e__VW.XView, 0 )+global.room_offset+88, __view_get( e__VW.YView, 0 )+32+16, string_hash_to_newline(global.GameOver));
            }
            if (drawStatus > 1)
            {
                draw_set_font(global.myFontSmall);
                draw_set_color(c_yellow);
                draw_text(__view_get( e__VW.XView, 0 )+global.room_offset+88, __view_get( e__VW.YView, 0 )+64+16, string_hash_to_newline(global.FinalScore));
            }
            if (drawStatus > 2)
            {
                draw_set_font(global.myFont);
                draw_set_color(c_white);
                draw_text(__view_get( e__VW.XView, 0 )+global.room_offset+88, __view_get( e__VW.YView, 0 )+72+16, string_hash_to_newline("$" + string(moneyCount)));
                ...
                if (global.customLevel)
                {
                    if (global.testLevel != "")
                    {
                        if (global.gamepadOn) str = global.Press + scrGetJoy(global.joyAttackVal) + global.ToEditLevel;
                        else str = global.Press + scrGetKey(global.keyAttackVal) + global.ToEditLevel;
                    }
                    else
                    {
                        if (global.gamepadOn) str = global.Press + scrGetJoy(global.joyAttackVal) + global.ToLoadAnotherLevel;
                        else str = global.Press + scrGetKey(global.keyAttackVal) + global.ToLoadAnotherLevel;
                    }
                ...
                {
                    if (global.gamepadOn) str = global.Press + scrGetJoy(global.joyAttackVal) + global.ForHighScores;
                    else str = global.Press + scrGetKey(global.keyAttackVal) + global.ForHighScores;
                    strLen = string_length(str)*8;
                    n = global.display_w - strLen;
                    n = ceil(n / 2);
                    draw_text(__view_get( e__VW.XView, 0 )+n, __view_get( e__VW.YView, 0 )+120, string_hash_to_newline(str));
        ...
        else if (isRoom("rSun"))
        {
            if (drawStatus > 0)
            {
                draw_set_font(global.myFont);
                draw_set_color(c_yellow);
                draw_text(__view_get( e__VW.XView, 0 )+global.room_offset+88, __view_get( e__VW.YView, 0 )+32+16, string_hash_to_newline(global.Finished));
            }
            if (drawStatus > 1)
            {
                draw_set_font(global.myFontSmall);
                draw_set_color(c_yellow);
                if (oSunRoom.highscore) str = global.YouSetANewRecord;
                else str = global.BetterLuckNextTime;
                strLen = string_length(str)*8;
                n = global.display_w - strLen;
                n = ceil(n / 2);
                draw_text(n-global.room_offset, __view_get( e__VW.YView, 0 )+64+16, string_hash_to_newline(str));
            }
        }
        else if (isRoom("rMoon"))
        {
            if (oMoonRoom.timer < 0)
            {
                ...
                if (drawStatus > 1)
                {
                    draw_set_font(global.myFontSmall);
                    draw_set_color(c_yellow);
                    if (oMoonRoom.highscore) str = global.YouSetANewRecord;
                    else str = global.BetterLuckNextTime;
                    strLen = string_length(str)*8;
                    n = global.display_w - strLen;
                    n = ceil(n / 2);
        ...
        else if (isRoom("rStars"))
        {
            ...
            if (drawStatus > 1)
            {
                draw_set_font(global.myFontSmall);
                draw_set_color(c_yellow);
                if (oStarsRoom.highscore) str = global.YouSetANewRecord;
                else str = global.BetterLuckNextTime;
                strLen = string_length(str)*8;
                n = global.display_w - strLen;
                n = ceil(n / 2);
