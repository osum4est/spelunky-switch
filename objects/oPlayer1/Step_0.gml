    ...
    sprite_index != sPExit and sprite_index != sDamselExit and sprite_index != sTunnelExit)
{
    ...
    if (isRoom("rOlmec") and holdItem)
    {
        ...
        else if (holdItem.type == global.Bomb)
        {
            if (holdItem.armed)
            {
                holdItem.held = false;
        ...
        else if (holdItem.type == global.Rope)
        {
            global.rope += 1;
            with holdItem { instance_destroy(); }
...
if (global.mobileBuild == true)
{
    if (keyboard_check_pressed(ord("F")))
        ...
        {
            global.touchControlsVisible = 2;
        }
        else if (global.touchControlsVisible == 2)
        {
            global.touchControlsVisible = 0;
        }
...
if (isLevel() and active and kPayPressed and not dead and not stunned)
{
    if (isInShop(x, y) and instance_exists(oShopkeeper))
    {
        ...
        if (holdItem)
        {
            ...
            {
                global.message = global.YouHaventGotEnoughMoney;
                global.message2 = "";
                global.messageTimer = 80;
            ...
            (not global.blackMarket and oShopkeeper.style == "Craps"))
        {
            ...
            else if (bet == 0 and global.money >= (1000 + global.currLevel * 500))
            {
                ...
                global.message = global.YouBetS + string(1000 + global.currLevel * 500) + "!";
                global.message2 = global.NowRollTheDice;
                global.messageTimer = 200;
            }
            else if (bet > 0)
            {
                global.message = global.OneBetAtATime;
                global.message2 = global.PleaseRollTheDice;
                global.messageTimer = 200;
            }
            else
            {
                global.message = global.YouNeedS + string(1000 + global.currLevel * 500) + " TO BET!";
                global.message2 = "";
                global.messageTimer = 200;
            }
        }
        ...
        if (oShopkeeper.style == "Kissing" and distance_to_object(oDamsel) < 16)
        {
            ...
            else if (n == 0 and global.money >= getKissValue())
            {
                if (obj.forSale and not obj.held)
                {
                    ...
                    global.plife += 1;
                    if (global.isDamsel) global.message = global.NowAintHeSweet;
                    else global.message = global.NowAintSheSweet;
                    global.message2 = "";
                    global.messageTimer = 200;
                }
            }
            ...
            {
                if (n == 0) global.message = global.YouNeedS + string(getKissValue()) + "!";
                else global.message = global.YouNeedS + string(obj.cost) + "!";
                global.message2 = global.GetOuttaHereDeadbeat;
                global.messageTimer = 200;
            }
        }
    }
...
if (isLevel() or isRoom("rSun") or isRoom("rMoon") or isRoom("rStars"))
{
    if (not dead and global.plife < 1)
    {
        if (global.hasAnkh)
        {
            ...
            global.message = global.TheAnkhShatters;
            global.message2 = global.YouHaveBeenRevived;
