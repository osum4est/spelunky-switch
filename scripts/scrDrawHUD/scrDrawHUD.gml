...
if (global.drawHUD && instance_exists(oPlayer1) && view_current==7)
{
    ...
    life = global.plife;
    if (life < 0) life = 0;
    draw_text(lifeX+16, vyv+8, string_hash_to_newline(life));
    if (global.hasStickyBombs) draw_sprite(sStickyBombIcon, -1, bombX, vyv+8);
    else draw_sprite(sBombIcon, -1, bombX, vyv+8);
    draw_text(bombX+16, vyv+8, string_hash_to_newline(global.bombs));
    draw_sprite(sRopeIcon, -1, ropeX, vyv+8);
    draw_text(ropeX+16, vyv+8, string_hash_to_newline(global.rope));
    draw_sprite(sDollarSign, -1, moneyX, vyv+8);
    draw_text(moneyX+16, vyv+8, string_hash_to_newline(global.money));
    ...
    if (global.collect > 0) draw_text(moneyX, vyv+8+16, string_hash_to_newline("+" + string(global.collect)));
