...
//show_debug_message("j="+string(kJump)+", jp="+string(kJumpPressed)+", jr="+string(kJumpReleased) );
...
{
    //DFK got rid of oCape = false reference, finding instance
    if (instance_exists(oCape)) {
        cape = instance_find(oCape,0);
        cape.open = false
    }
    kJumped = false;
}
